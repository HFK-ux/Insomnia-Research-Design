...
folder = "/Users/harunfurkankanik/Desktop/targetDetect/targetDetect_insomnia";
...
insomnia_mean_accuracy = zeros(num_subjects, num_conditions);
insomnia_mean_rt = zeros(num_subjects, num_conditions);
...
for subj = 1:num_subjects
    ...
    for cond = 1:num_conditions
        insomnia_mean_accuracy(subj, cond) = mean(correct_responses{cond}); % Mean accuracy for this condition
        insomnia_mean_rt(subj, cond) = mean(reaction_times{cond}(correct_responses{cond} == 1)); % Mean RT for correct responses
    end
end

save("Insomnia_group_corrected_data", "insomnia_mean_accuracy", "insomnia_mean_rt");
...
reaction_times = reshape(insomnia_mean_rt, [], 1); % Converts 35x3 to a single column vector
accuracy_scores = reshape(insomnia_mean_accuracy, [], 1);

% Create a grouping variable for distractor levels
group_labels = [ones(size(insomnia_mean_rt, 1), 1); ones(size(insomnia_mean_rt, 1), 1) * 2; ones(size(insomnia_mean_rt, 1), 1) * 3];
...
% The ANOVA for accuracy was not significant, so multiple comparisons were
% not performed.
...
% 10 - 15 Distractors
[H P CI STAT] = ttest(insomnia_mean_rt(:, 1), insomnia_mean_rt(:, 2), 'Alpha', alpha_corrected)

% 10 vs 20 Distractors
[H P CI STAT] = ttest(insomnia_mean_rt(:, 1), insomnia_mean_rt(:, 3), 'Alpha', alpha_corrected)

% 15 vs 20 Distractors
[H P CI STAT]= ttest(insomnia_mean_rt(:, 2), insomnia_mean_rt(:, 3), 'Alpha', alpha_corrected)

%% Plotting

figure;
bar(mean(insomnia_mean_rt, 1), 'FaceColor', [0.2 0.6 0.8]);
hold on;
errorbar(mean(insomnia_mean_rt, 1), std(insomnia_mean_rt, [], 1) / sqrt(size(insomnia_mean_rt, 1)), 'k.', 'LineWidth', 1.5);
xticklabels({'10', '15', '20'});
xlabel('Number of Distractors');
ylabel('Reaction Time (ms)');
title('Insomnia Group Reaction Times for Different Distractor Levels');
grid on;
hold off;

figure;
bar(mean(insomnia_mean_accuracy, 1), 'FaceColor', [0.8 0.4 0.2]);
hold on;
errorbar(mean(insomnia_mean_accuracy, 1), std(insomnia_mean_accuracy, [], 1) / sqrt(size(insomnia_mean_accuracy, 1)), 'k.', 'LineWidth', 1.5);
xticklabels({'10', '15', '20'});
xlabel('Number of Distractors');
ylabel('Accuracy (%)');
